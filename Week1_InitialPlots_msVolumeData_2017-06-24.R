...
# Let's look at just the 6 months of Jul-Dec 2016
daily_6months <- daily[130:253,]

# PLOT 03: Same plot as before except just three months:
ggplot(data=daily_6months, aes(x=date, y=mss, group=1)) +
  geom_line(color = "forestgreen", size = 1) +
  # Plot and title labels
  xlab("Date") +
  ylab("Manuscripts Received") +
  ggtitle("Daily Manuscript Volume - 7/1/2016 to 12/31/2016") +
  # "The Economist" magazine theme
  theme_economist() +
  # Here we adjust all the font sizes, angles, and justifications in a 
  # theme() call.
  ...
  scale_y_continuous(limits = c(0,300), breaks=seq(0,300,100)) + 
  scale_x_date(breaks = date_breaks("2 weeks"), labels = date_format("%Y-%m-%d"))
